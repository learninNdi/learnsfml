        ...
        while (const std::optional event = window.pollEvent())
        {
            ...
            else if (const auto *mouseButtonPressed = event->getIf<sf::Event::MouseButtonPressed>())
            {
                if (mouseButtonPressed->button == sf::Mouse::Button::Left)
                {
                    ...
                    std::mt19937 rng(rd());
                    float r = std::uniform_real_distribution(0.f, 50.f)(rng);
                    if (mouseButtonPressed->position.x > 0 && mouseButtonPressed->position.x < SCR_WIDTH - r && mouseButtonPressed->position.y > 0 && mouseButtonPressed->position.y < SCR_HEIGHT - r)
                    {
                        float x = mouseButtonPressed->position.x;
                        float y = mouseButtonPressed->position.y;
                        float vx = std::uniform_real_distribution(-100.f, 100.f)(rng);
                        float vy = std::uniform_real_distribution(-100.f, 100.f)(rng);
                        float m = r;
                        sf::Color color(rand() % 255, rand() % 255, rand() % 255);

                        balls.emplace_back(r, sf::Vector2f({x, y}), sf::Vector2f({vx, vy}), m, color);
                    }
                }
            }
        }
